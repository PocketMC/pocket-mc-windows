...
namespace PocketMC.Infrastructure.Mods
{
    public static class JavaModMetadataService
    {
        ...
        /// This is intentionally conservative â€” it is not a full TOML parser, but
        ...
        private static IEnumerable<(string Line, string Section)> EnumerateActiveTomlLines(string toml)
        {
            ...
            foreach (string rawLine in toml.Split('\n'))
            {
                ...
                if (inMultiLineString)
                {
                    ...
                    // Either way, skip this line â€” it is string content, not a key = value pair.
                    continue;
                }
                ...
                if (trimmed.Contains("'''") || trimmed.Contains("\"\"\""))
                {
                    ...
                    if (secondIdx < 0)
                    {
                        // Opening delimiter without closing on the same line â€” entering multi-line string.
                        inMultiLineString = true;
                        multiLineDelimiter = delimiter;
                        continue;
                    }
                    // Both delimiters on same line â€” this is a single-line triple-quoted string.
